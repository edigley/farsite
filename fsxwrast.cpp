...
void Rasterize::raster()
{
	...
	GetRastRes(&RastResX, &RastResY);
	if (ExNumPts > 0)				// TERMINATION OF INWARD BURNING FIRE
	{
		AllocPerimeter1(count, ExNumPts);
		SetNumPoints(count, ExNumPts);
		for (inc = 0; inc < ExNumPts; inc++)		 // fill in perim1 array
		{
			xpt1 = GetPerimeter2Value(inc, XCOORD);
			ypt1 = GetPerimeter2Value(inc, YCOORD);
			SetPerimeter1(count, inc, xpt1, ypt1);
			ros1 = GetPerimeter2Value(inc, ROSVAL);
			fli1 = GetPerimeter2Value(inc, FLIVAL);
			rcx1 = GetPerimeter2Value(inc, RCXVAL);
			SetFireChx(count, inc, ros1, fli1);
			SetReact(count, inc, rcx1);
		}
		...
		for (E = 0; E <= noeast; E++)
		{
				...
				N++)  		  // XUtilities public data members
			{
				...
				if (inside)
				{
					xpt1 = GetPerimeter1Value(count, 0, XCOORD);
					ypt1 = GetPerimeter1Value(count, 0, YCOORD);
					r = GetPerimeter1Value(count, 0, ROSVAL);
					f = GetPerimeter1Value(count, 0, FLIVAL);
					rx = GetPerimeter1Value(count, 0, RCXVAL);
					...
					{
						xpt1 = GetPerimeter1Value(count, inc, XCOORD);
						ypt1 = GetPerimeter1Value(count, inc, YCOORD);
						dist = pow2(xpt1 - startx) + pow2(ypt1 - starty);
						if (dist < lowdist) 	   	   // select the closest point values
						{
							lowdist = dist;
							r = GetPerimeter1Value(count, inc, ROSVAL);
							f = GetPerimeter1Value(count, inc, FLIVAL);
							rx = GetPerimeter1Value(count, inc, RCXVAL);
							if (lowdist > 0.0)
							{
								xpt1n = xpt1;
								ypt1n = ypt1;
		...
		nump = GetNumPoints(count);  	  // exit raster procedure
	if (nump != 0)
	{
		GetPerimeter2(0, &xpt1, &ypt1, &ros1, &fli1, &rcx1);
		xpt2 = GetPerimeter1Value(count, 0, XCOORD);
		ypt2 = GetPerimeter1Value(count, 0, YCOORD);
		ros2 = GetPerimeter1Value(count, 0, ROSVAL);
		fli2 = GetPerimeter1Value(count, 0, FLIVAL);
		rcx2 = GetPerimeter1Value(count, 0, RCXVAL);
		for (inc = 1; inc <= nump; inc++)
		{
			if (inc < nump)
				...
				newnump = 0;
			GetPerimeter2(newnump, &xpt1n, &ypt1n, &ros1n, &fli1n, &rcx1n);
			xpt2n = GetPerimeter1Value(count, newnump, XCOORD);
			ypt2n = GetPerimeter1Value(count, newnump, YCOORD);
			ros2n = GetPerimeter1Value(count, newnump, ROSVAL);
			fli2n = GetPerimeter1Value(count, newnump, FLIVAL);
			rcx2n = GetPerimeter1Value(count, newnump, RCXVAL);
			if (ros1 < 0.0)
			{
	...
	bool ViewPort, long ThemeNumber)
{
	...
	if (AccessOutputUnits(GETVAL))		// if English Output Units
	{
	...
	CurTime = GetTemporaryTimeStep();
	LastTime = 0;
	PFRes = Resolution;
	if (ViewPort)
	{
		PFNorth = GetViewNorth();
	...
	{
		xmin = GetPerimeter1Value(i, GetNumPoints(i), XCOORD);
		xmax = GetPerimeter1Value(i, GetNumPoints(i), YCOORD);
		ymin = GetPerimeter1Value(i, GetNumPoints(i), ROSVAL);
		ymax = GetPerimeter1Value(i, GetNumPoints(i), FLIVAL);
		starty = ymax;
		do
		{
			startx = xmin;
				...
				for (E = 0; E < noeast; E++)
				{
						...
						N++)   		 	// XUtilities public data members
					{
						...
						if (inside)
						{
							...
							{
							case 0:
								map[loc] = WrT * ConvHeat;	// MJ, or MBtu
								break;
							case 1:
								map[loc] = WrT * ConvWt;
								break;
	...
	fprintf(outfile, "xllcorner %lf\n", ConvertEastingOffsetToUtm(PFWest));
	fprintf(outfile, "yllcorner %lf\n", ConvertNorthingOffsetToUtm(PFSouth));
...
void Rasterize::FMaxMin(int Coord, double* ptmax, double* ptmin)
{
	...
	*ptmax = *ptmin = GetPerimeter2Value(0, Coord);
	for (long i = 1; i < ExNumPts; i++)
	{
		pt2 = GetPerimeter2Value(i, Coord);
		if (*ptmax > pt2)
			*ptmax = pt2;
		if (*ptmin < pt2)
			*ptmin = pt2;
