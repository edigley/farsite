...
ShapeFileUtils::~ShapeFileUtils()
{
    if (ShapeFileName)
        free(ShapeFileName);
    if (DataBaseName)
        delete[] DataBaseName;//free(DataBaseName);
    //FreeSHPArrays();
}
...
void ShapeFileUtils::InitializeShapeExport(char* FileName, long poly)
{
    ...
    if (ShapeFileName[len - 4] == '.')
    {
        ...
        //							(len + 1) * sizeof(char));
        DataBaseName = new char [len+1];//(char *)malloc((len + 1) * sizeof(char));
    ...
    else
    {
        //DataBaseName = (char *) GlobalAlloc(GMEM_FIXED | GMEM_ZEROINIT,
        //							(len + 5) * sizeof(char));
        DataBaseName = new char[len+5];//(char *) malloc((len + 5) * sizeof(char));
    ...
    for (i = 0; i < nEntities; i++)
    {
        ...
        if (pSHP->nVertices > 0)
        {
            for (k = 0; k < pSHP->nParts; k++)
            {
                ...
                if (end - start <= 0)
                    continue;
                AllocPerimeter1(GetNewFires(), (end - start) + 1);
                for (j = start; j < end; j++)
                {
                    xpt = ConvertUtmToEastingOffset(pSHP->padfX[j]);
                    ypt = ConvertUtmToNorthingOffset(pSHP->padfY[j]);
                    SetPerimeter1(GetNewFires(), j - start, xpt, ypt);
                    SetFireChx(GetNewFires(), j - start, -1.0, fli);
                    if (j == start)
                    {
                        minx = maxx = xpt;
                        miny = maxy = ypt;
                ...
                SetPerimeter1(GetNewFires(), j - start, minx, maxx);
                SetFireChx(GetNewFires(), j - start, miny, maxy);
                SetInout(GetNewFires(), PolygonType);
                SetNumPoints(GetNewFires(), end - start);//pSHP->nVertices);
                IncNewFires(1);
            }
        }
...
void ShapeFileUtils::RetrieveExportFileInfo()
{
    ...
    if (DataBaseName)
        delete[] DataBaseName;//free(DataBaseName);
    ShapeFileName = strdup(GetShapeFileChx(&VisOnly, &Polygons, &BarSeparate));
    len = strlen(ShapeFileName);
    if (ShapeFileName[len - 4] == '.')
    {
        //DataBaseName = (char *) GlobalAlloc(GMEM_FIXED | GMEM_ZEROINIT,
...
bool ShapeFileUtils::ShapeData(long CurrentFire, double SimTime)
{
    ...
    for (count = 0; count < NumVertex; count++)
    {
        ...
        {
            xpt = GetPerimeter2Value(count, XCOORD);
            ypt = GetPerimeter2Value(count, YCOORD);
            //ros=0.0;
        }
        VertexX[count] = ConvertEastingOffsetToUtm(xpt);
        VertexY[count] = ConvertNorthingOffsetToUtm(ypt);
        //VertexZ[count]=ros;
    }
...
void ShapeFileUtils::ConvertSimTime(double SimTime)
{
    ...
    do
    {
        ...
        case 1:
            days = 31;
            break;			 // days in each Monthnth, ignoring leap year
        case 2:
            days = 28;
            break;
        case 3:
