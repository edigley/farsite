	...
	while (TimeStep > 0.0)
	{
		...
		if (fli2 < 0.0 || fli2n < 0.0)	// can't suppress extinguished fire
		{
			if (fli1n == 0.0)
				fli1n = 0.001;
			if (fli1n > 0.0)
				SetFireChx(attack->FireNumber, attack->CurrentPoint, ros1n,
					-fli1n);
			...
			for (i = 1; i < GetNumPoints(attack->FireNumber); i++)
			{
				if (attack->Reverse)
				{
					...
					if (j < 0)
						j = GetNumPoints(attack->FireNumber) - i;
				}
				else
				{
				...
				if (fli1 >= 0.0)
				{
					...
					if (fli1n >= 0.0)
					{
						...
						GetPerimeter2(attack->CurrentPoint, &xpt1, &ypt1,
							&ros1, &fli1, &rcx1);
			...
			if (i == GetNumPoints(attack->FireNumber))	// searched fire and no unburned points found
				return false;
			else
				continue;
		}
		...
		FuelType = GetFuelConversion(tempcell.f);	//CheckAltFuels(tempcell.f, xpt1, ypt1)); no longer valid for version 4.0
		if (FuelType > 0)
		{
			LineRate = crew[attack->CrewNum]->LineProduction[FuelType - 1] * MetricResolutionConvert();
		...
		if ((posit = ProblemQuad()) > 0)
		{
			...
			else
			{
				...
				//LineDist-=HorizPerimDist;
				//if(fli1<0)
				//     SetPerimeter2(attack->CurrentPoint, xpt1, ypt1, ros1, -fli1);
				ReplaceX1n = xpt1n;
				ReplaceY1n = ypt1n;
				ReplaceR1n = ros1n;
				ReplaceF1n = fli1n;
		...
		/*
			if(attack->Reverse)
			{    fli1=GetPerimeter1Value(attack->FireNumber, attack->CurrentPoint, FLIVAL);
				 if(fli1>0.0)
			  {	ros1=GetPerimeter1Value(attack->FireNumber, attack->CurrentPoint, ROSVAL);
				SetFireChx(attack->FireNumber, attack->NextPoint, ros1, fli1);
				}
			}
			else
			{	fli1=GetPerimeter1Value(attack->FireNumber, attack->NextPoint, FLIVAL);
			   if(fli1>0.0)
			  {	ros1=GetPerimeter1Value(attack->FireNumber, attack->NextPoint, ROSVAL);
				SetFireChx(attack->FireNumber, attack->NextPoint, ros1, fli1);
				}
			}
			*/
...
bool Attack::IndirectAttack(AttackData* atk, double TimeStep)
{
	...
	while (TimeStep > 0.0)    // perform distance checking in line building rate
	{
		...
		CellData(xpt1, ypt1, tempcell, tempcrown, tempground, &posit);
		FuelType = GetFuelConversion(tempcell.f); //CheckAltFuels(tempcell.f, xpt1, ypt1));
		if (FuelType > 0)
			LineRate = crew[attack->CrewNum]->LineProduction[FuelType - 1] * MetricResolutionConvert();
		else
			LineRate = 0.0;
	...
	if (posit > 0)
	{
		...
		if (attack->FireNumber > -1)  // if not first time through
		{
			...
			//SetInout(attack->FireNumber, 0);
			//SetNumPoints(attack->FireNumber, 0);
			//IncSkipFires(1);
		}
		else
		{
...
bool Attack::ParallelAttack(AttackData* atk, double TimeStep)
{
	...
	if ((Hull2 = new double[GetNumPoints(attack->FireNumber) * 6]) == NULL) //(double *) calloc(GetNumPoints(attack->FireNumber)*6, sizeof(double)))==NULL)
		return false;
	GetConvexHull(Hull2, &NumHullPts);
	if ((Hull1 = new double[GetNumPoints(attack->FireNumber) * 4]) == NULL) //(double *) calloc(GetNumPoints(attack->FireNumber)*4, sizeof(double)))==NULL)
	{
		if (Hull2)
	...
	while (TimeStep > 0.0)
	{
		...
		if (fli2 <= 0.0)	// can't suppress extinguished fire
		{
			...
			{
				fli2n = Hull2[i * 3 + 2];  //GetPerimeter2Value(j, FLIVAL);
				if (fli2n > 0.0)
					break;
			}
...
void Attack::BoundingBox()
{
	...
	long NumFire = attack->FireNumber;//GetNewFires()-1;
	...
	double nfli, double nrcx, long InsertType)
{
	...
	for (i = 0; i < numpts; i++)
	{
		...
		attack->IndirectLine2[(i + j) * 4] = GetPerimeter1Value(numfire, i,
												XCOORD);
		attack->IndirectLine2[(i + j) * 4 + 1] = GetPerimeter1Value(numfire,
													i, YCOORD);
		attack->IndirectLine2[(i + j) * 4 + 2] = GetPerimeter1Value(numfire,
													i, ROSVAL);
		attack->IndirectLine2[(i + j) * 4 + 3] = GetPerimeter1Value(numfire,
													i, FLIVAL);
		attack->IndirectLine2[(i + j) * 4 + 4] = GetPerimeter1Value(numfire,
													i, RCXVAL);
	}
	if (j == 0)
	{
...
void Attack::BurnOut()
{
	...
	DistRes = GetDistRes() / 1.2 * MetricResolutionConvert();	// make a little finer
	...
	//SetNumPoints(GetNewFires(), 2*NewPoints);
	if (k < NewPoints)	// no
	{
	...
	double* ybuf2)
{
	...
	DistanceResolution = GetDistRes() * 0.6 * MetricResolutionConvert();	// can't reduce width because center
	//DistanceResolution=GetDistRes()*1.2*MetricResolutionConvert();	
	...
	for (j = ConvexStartPt; j < ConvexEndPt; j++)
	{
		...
		if (j == ConvexEndPt - 1 && ConOrder > -1)
		{
			...
			/*if(labs(ConPt-OldPt)>1)
					{	xpt=GetPerimeter1Value(FireNum, ConPt, XCOORD);
					   ypt=GetPerimeter1Value(FireNum, ConPt, YCOORD);
