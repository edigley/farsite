...
double Crown::CrownSpread(double avgros, double CrownFireSpreadRate)	     // R10 is from Rothermel 1991
{
	double CrownROS=0.0;

	if(CrownBulkDensity>0.0)
	{    CrownROS=avgros+CrownFractionBurned*(CrownFireSpreadRate-avgros);	// Van Wagner's 2-equation crown fire model
     	if(GetCrownFireCalculation()==0)     // do only for original FARSITE calculations
		{
...
{
	// uses original crown fire spread calculations from Finney 1998
     double a;
...
double Crown::CrownBurn2(double avgros, double fli, double AccelConstant, void *vt)
{
	// uses crown fire calculations from Scott and Reinhardt 2001
     double R10, Rratio, Eratio, Ractual, Rsa;
     double cw1, cw2, cs, cpw1, cpw2, X, inc, CritWind;
     double sw1, sw2, ss, spw1, spw2;
     BurnThread *bt;
