...
GroupAirAttack::GroupAirAttack(GroupAttackData* atk)
{
	attack = atk;
}
...
bool GroupAirAttack::SetGroupAssignment(double* line, long FireNum, bool Reset)
{
	...
	else				  // positive number indicating actual fire number, line indicates 1st pt
	{
		...
		pt -= 1;
		if (pt < 0)
			pt += GetNumPoints(FireNum);
		x = GetPerimeter1Value(FireNum, pt, XCOORD);
		y = GetPerimeter1Value(FireNum, pt, YCOORD);
		dist = pow2(x - line[2]) + pow2(y - line[3]);
		pt += 2;
		if (pt > GetNumPoints(FireNum) - 1)
			pt -= GetNumPoints(FireNum);
		x = GetPerimeter1Value(FireNum, pt, XCOORD);
		y = GetPerimeter1Value(FireNum, pt, YCOORD);
...
bool GroupAirAttack::ExecuteAttacks(double TimeIncrement)
{
	...
	for (i = 0; i < attack->NumCurrentAircraft; i++)
	{
		...
		{
			if (aircraft[attack->AircraftNumber[i]]->Units == 1)
				Mult = 0.3048;  	  // feet to meters

			XDist = YDist = 0.0;
			MinDist = aircraft[attack->AircraftNumber[i]]->PatternLength[attack->CoverageLevel[i]] * Mult * MetricResolutionConvert();
			if (attack->FireNumber < 0) 	 // indirect attack
			{
				...
				if (fabs(attack->WaitTime[i] -
						aircraft[attack->AircraftNumber[i]]->ReturnTime) <=
					1e-3)
					attack->WaitTime[i] = aircraft[attack->AircraftNumber[i]]->ReturnTime;
			}
			else				  // direct attack, get current position on fire
			{
				...
				for (j = 0; j < GetNumPoints(attack->FireNumber); j++)
				{
					InitDist = pow2(attack->IndirectLine[0] -
								GetPerimeter1Value(attack->FireNumber, j,
									XCOORD)) +
				...
				AttackLine[2] = GetPerimeter1Value(attack->FireNumber, NextPt,
									XCOORD);
				AttackLine[3] = GetPerimeter1Value(attack->FireNumber, NextPt,
									YCOORD);
			...
			SetupAirAttack(attack->AircraftNumber[i],
