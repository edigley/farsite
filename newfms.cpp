/*! \file newfms.cpp newfms.h 
    ...
    \author Copyright (C) 1997-2004 by Collin D. Bevins.
...
{
	return (Fms_CreateStick(name,
				/* Name for the new stick instance. 				  */
...
void Fms_Destroy(Fms* fms)
{
	...
	if (fms->x)
		delete[] fms->x;//free( fms->x );
	if (fms->d)
		delete[] fms->d;//free( fms->d );
	if (fms->t)
		delete[] fms->t;//free( fms->d );
	if (fms->s)
		delete[] fms->s;//free( fms->s );
	if (fms->w)
		delete[] fms->w;//free( fms->w );
	if (fms->v)
		delete[] fms->v;//free( fms->v );
	fms->name = 0;
	fms->x = 0;
	fms->d = 0;
	fms->t = 0;
...
static void Fms_Diffusivity(Fms* fms)
{
	...
	for (i = 0; i < fms->n; i++)
	{
		...
		dvpr = 18.0 * 0.016 * (1.0 - vfcw) * dv * ps1 * dhdm /
			(fms->dp * 1.987 * tk);
		fms->d[i] = dvpr +
			3600. * 0.0985 * con * fac * exp(-e / (1.987 * tk));
	}
	return;
}
...
void Fms_Initialize(Fms* fms, /* Pointer to fuel moisture stick object. 	*/
...
void Fms_InitializeAt(Fms* fms,
	/* Pointer to fuel moisture stick object.     */
...
void Fms_UpdateAt(Fms* fms, /* Pointer to the fuel moisture stick object. */
	int    year, /* Year of the update  			(4 digits) */
	int    month, /* Month of the update		(1=Jan, 12=Dec) */
	int    day, /* Day of the update				   (1-31) */
	int    hour, /* Hour of the update  				(0-23) */
	...
	double rcumCm   			/* New cumulative rainfall  			 (cm) */)
{
	...
	if (!fms->init)
	{
		Fms_InitializeAt(fms, year, month, day, hour, minute, second,
			millisecond, aC, hFtn, sW, rcumCm, aC, hFtn, 0.5 * fms->wmx);
	}
...
void Fms_Update(Fms* fms, /* Pointer to the fuel moisture stick object. */
	double eHr, /* Elapsed time since last observation    (h) */
	double aC, /* New air temperature   				(oC) */
	double hFtn, /* New air humidity					 (g/g) */
	double sW,
	...
	double rcumCm   			/* New cumulative rainfall  			 (cm) */)
{
	...
	for (tt = fms->mdt; tt <= eHr + fms->mdt; tt += fms->mdt)
	{
			...
			for (i = 0; i < fms->n; i++)
			{
				...
				if (par > 0.)
				{
					...
					if (par > Sir && par < Scr)
					{
						...
						{
							gnu = 0.00439 + 0.00000177 * pow((338.76 - tkf),
															2.1237);
						...
						g[i] = (ak / (gnu * wdiff)) * fms->x[i] * fms->vf * pow((Scr /
							fms->s[i]),
																				1.5) ;
					}
				}   /* end if par > 0. */
		...
		for (i = 1; i < fms->n - 1; i++)
		{
			...
			ar = fms->x[i] * fms->dx / fms->mdt;
			ap = ae + aw + ar;
			fms->t[i] = (ae * told[i + 1] + aw * told[i - 1] + ar * told[i]) /
				ap;
			if (fms->t[i] > 71.)
			{
				fms->t[i] = 71.;
