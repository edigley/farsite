...
void CrossThread::CrossCompare()
{
	...
	//printf("Num fires:%d",GetNumFires());
...
// 	tmpNumCross_array = (long*)calloc(threads, sizeof(long));
// 	int w=0;
// 	for (w=0;w<threads;w++){
// 		  tmpNumCross_array[w]=0;
// 		  
// 		}
	...
	do
	{
		...
		//printf("StartA:%d EndA:%d\n",SpanAStart, SpanAEnd);
 		...
 		//#pragma omp parallel for ordered private(ypt,xptn,yptn,xg,yg,i,k,ros1,fli1,rcx1,xptl, yptl,crxpt, crypt, crxptl, cryptl, crxptn, cryptn,ros2, ros3,fli2, fli3, fli4,rcx2, rcx3, j, m, n, dup1, dup2,crossyn, ptcross1, ptcross2,SinglePtCross,newx, newy) private(tmpNumCross)
		for (i = SpanAStart;i < SpanAEnd;i++)   // for each point in a given fire
		{
			//printf("Soy %d y Numthreads:%d\n",omp_get_thread_num(),omp_get_num_threads());
...
// 			}
			...
			//#pragma omp parallel for private(xptl, yptl,crxpt, crypt, crxptl, cryptl, crxptn, cryptn,ros2, ros3,fli2, fli3, fli4,rcx2, rcx3, j, m, n, dup1, dup2,crossyn, ptcross1, ptcross2,SinglePtCross,newx, newy)
			for (j = SpanBStart;j < SpanBEnd;j++)	// FOR ALL FIRES CHECK FOR ALL CROSSES FROM THE FIRST ELEMENT
			{
				...
				if (crossyn == true)
				{
					...
					{
						if (CurrentFire == NextFire)
							SetIntersection(tmpNumCross, -i, -j);	// point of order i in first fire, j on second fire
						else
							SetIntersection(tmpNumCross, i, j);	// point of order i in first fire, j on second fire
					}
					else
					...
					SetInterPoint(tmpNumCross, newx, newy, ros3, fli3, rcx3);	 //write intersected points
					if (GetInout(CurrentFire) == 2 && GetInout(NextFire) == 1)
					{
						if (fli2 < 0.0) 	   // check for stationary points on outward fire
						{
							//tmpNumCross_array[omp_get_thread_num()]=0;
							tmpNumCross=0;// if found dont merge with inward burning fire

							//CrossError=true;
						}
					}
					//tmpNumCross_array[omp_get_thread_num()]++;
					tmpNumCross++;// allow outward fire to get only outward fire perimeter
				}
			    crossyn = 0;
			}
		}
...
// 	for (w=0;w<threads;w++){
// 	      if (tmpNumCross_array[w]==0){
// 		tmpNumCross=0;
// 	      } else{
// 		tmpNumCross=+tmpNumCross_array[w];
// 	      }	  
// 	}
		NumCross=tmpNumCross;
	...
	double* newy, long* dup1, long* dup2)
{
	...
	*dup1 = *dup2 = 0;
	if (fabs(slope1 - slope2) < 1e-9)
	{
		...
		{
		 /* if (xdiff1 == 0.0) {counter++;};
		   if (xdiff2 == 0.0) {counter2++;};
		    if (OnSpan1) {counter3++;};
		    if (OnSpan2) {counter4++;};
		    if (ypt1 <= ypt1n) {counter5++;};
		    if (ypt1 > ypt2n) {counter6++;};
		    if (ypt1 >= ypt1n) {counter7++;};
		    if (ypt1 < ypt2n) {counter8++;};
		    
		    if (ypt1n <= ypt1) {counter9++;};
		    if (ypt1n > ypt2) {counter10++;};
		    if (ypt1n >= ypt1) {counter11++;};
		    if (ypt1n < ypt2) {counter12++;};*/
