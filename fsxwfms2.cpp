...
bool FireEnvironment2::CalcMapFuelMoistures(double SimTime)
{
	...
	for (i = 0; i < NumStations; i++)
	{
		...
		{
			if (Stations[i].ReAllocFuels(k) ||
				EnvironmentChanged(GETVAL, i, k))
			{
				FreeHistory(i, k);
				if (k == 1)
				{
...
bool FireEnvironment2::AllocStations(long Num)
{
	...
	if (Num > 0)
	{
		...
		for (i = 0; i < Num; i++)
		{
			Stations[i].AllocFuels((long)SIZECLASS_10HR);
			Stations[i].AllocFuels((long)SIZECLASS_100HR);
			Stations[i].AllocFuels((long)SIZECLASS_1000HR);
		}
	...
	double* humidmn, double* tempmx, double* tempmn)
{
 ...
 //OJO MODIFICADO! PROVOCA BUCLE INFINITO EN ALGUNOS CASOS !
	...
	double aspectf, long cover, double* equil, double* solrad, long FuelSize)	// millivolts
{
	...
	else if (Stations[sn].FMS[FuelSize][Stations[sn].FuelKey[FuelSize][fuel - 1] - 1]->FirstTime >
		Time ||
		Stations[sn].FMS[FuelSize][Stations[sn].FuelKey[FuelSize][fuel - 1] - 1]->LastTime <
		Time)
	{
		CheckMoistureHistory(sn, FuelSize, Time);
		if (FuelSize == SIZECLASS_10HR)
	...
	if (GetTheme_Units(E_DATA) == 0)
		loc = ((double) (elev - GetLoElev())) /
			(double) GetMoistCalcInterval(FuelSize,
															FM_INTERVAL_ELEV);
	else
		loc = ((double) (elev - GetLoElev() * 0.3048)) /
			(double)
			GetMoistCalcInterval(FuelSize,
				FM_INTERVAL_ELEV);
	...
	SlopeIndexN = SlopeIndex = slope / GetMoistCalcInterval(FuelSize,
										FM_INTERVAL_SLOPE);
	...
	AspIndexN = AspIndex = Aspect / GetMoistCalcInterval(FuelSize,
										FM_INTERVAL_ASP);
	...
	loc = (double)
		(Aspect - (AspIndex * GetMoistCalcInterval(FuelSize, FM_INTERVAL_ASP))) /
		(double)
		GetMoistCalcInterval(FuelSize,
			FM_INTERVAL_ASP);
	afract = modf(loc, &ipart);

	CovIndexN = CovIndex = cover / GetMoistCalcInterval(FuelSize,
									FM_INTERVAL_COV);
	...
	loc = (double)
		(cover - (CovIndex * GetMoistCalcInterval(FuelSize, FM_INTERVAL_COV))) /
		(double)
		GetMoistCalcInterval(FuelSize,
			FM_INTERVAL_COV);
						...
						for (p = 0; p < MxDesc.NumCovers[i][FuelSize]; p++)
						{
								...
								UseConditioningPeriod(GETVAL) == false)
							{
								...
								else
								{
									...
									Moist = Get1000HrMoisture(Stations[i].FMS[FuelSize][j]->Fuel, true) /100.0;
									if (Moist <= 0.10)
										Moist = 0.10;
									Stations[i].FMS[FuelSize][j]->Fms_Elev[k]->Fms_Slope[m]->
					...
					for (p = 0; p < MxDesc.NumCovers[i][q]; p++)
					{
						...
						else   // copy pointer from MxDesc to site data
						{
							...
							case SIZECLASS_1HR:
								break;
							case SIZECLASS_10HR:
								MxDesc.fms[loc] = Stations[i].FMS[q][j]->Fms_Elev[k]->Fms_Slope[m]->Fms_Aspect[n]->fms[p] = Fms_Create10Hour("10hr");
								break;
							case SIZECLASS_100HR:
								MxDesc.fms[loc] = Stations[i].FMS[q][j]->Fms_Elev[k]->Fms_Slope[m]->Fms_Aspect[n]->fms[p] = Fms_Create100Hour("100hr");
								break;
							case SIZECLASS_1000HR:
								MxDesc.fms[loc] = Stations[i].FMS[q][j]->Fms_Elev[k]->Fms_Slope[m]->Fms_Aspect[n]->fms[p] = Fms_Create1000Hour("1000hr");
								break;
							}
						}
					}
	...
	long FuelSize)
{
	...
	if (Stations[sn].FMS[FuelSize][FuelType]->LastTime == 0.0 ||
		MxDesc.NumHist[sn][FuelSize] == 0) //SimTime==GetActualTimeStep() ||
	{
...
void FuelMoistureMap::SearchCondenseFuels(long FuelSize)
{
	...
	else
	{
		for (i = 0; i < MAXNUM_COARSEWOODY_MODELS; i++)
		{
			...
			//t10 = (long)GetWoodyFuelMoisture(num, FuelSize);
               t10=(long)Get1000HrMoisture(num, true);
			for (j = 0; j < MAXNUM_COARSEWOODY_MODELS; j++)
			{
				...
				if (FuelKey[FuelSize][j] < FuelOrder)
					continue;
				num = (long)j + 1;
				//t10a = (long)GetWoodyFuelMoisture(num, FuelSize);
				t10a=(long)Get1000HrMoisture(num, true);
...
bool FuelMoistureMap::ReAllocFuels(long FuelSize)
{
	...
	if (FuelSize < SIZECLASS_1000HR)
	{
		...
		if (GetLandscapeTheme()->AllCats[3][numfuels] == 99)
			numfuels--;

		if (GetLandscapeTheme()->AllCats[3][numfuels] == 98)
			numfuels--;
	}
	else
	{
...
//	ZeroMemory(FuelKey[FuelSize], MAXNUM_FUEL_MODELS * sizeof(long));
	...
	numfuels = k - 1;				  	// new potential number, not condensed
     ...
     k=1;
     for(i=0; i<MAXNUM_FUEL_MODELS; i++)
     {    if(FuelKey[FuelSize][i]>0)
               FuelKey[FuelSize][i]=k++;
     }
...
bool FMS_Elevations::AllocElevations()
{
	...
	for (i = 0; i < NumElevs; i++)
	{
		...
		{
			Fms_Elev[i]->Elev = (long)(i * GetMoistCalcInterval(FuelSize, FM_INTERVAL_ELEV) + GetLoElev() * conv);
	...
	DeadMoistureDescription* mxdesc, bool firsttime)
{
	...
	if (ThreadOrder < 0)
	{
		...
		hFmsThread = (HANDLE) ::_beginthreadex(NULL, 0,
								&FmsThread::RunFmsThread, this, NULL,
								&ThreadID);
...
void FmsThread::UpdateMapMoisture()
{
	...
	if (FirstTime)
	{
		...
		else
		{
			...
			mx=Get1000HrMoisture(Stations[sn].FMS[FuelSize][i]->Fuel, true)/100.0;
			if (mx <= 0.10)
				mx = 0.10;
		}
		for (j = Begin; j < End; j++)   // for each elevation
		{
			...
			humid /= 100.0;
			for (k = 0;
				k < Stations[sn].FMS[FuelSize][i]->Fms_Elev[j]->NumSlopes;
				k++)
			{
				Slope = Stations[sn].FMS[FuelSize][i]->Fms_Elev[j]->LoVal +
					k * GetMoistCalcInterval(FuelSize,
							FM_INTERVAL_SLOPE);
				for (m = 0;
					m <
					Stations[sn].FMS[FuelSize][i]->Fms_Elev[j]->Fms_Slope[k]->NumAspects;
					m++)
				{
					Aspect = Stations[sn].FMS[FuelSize][i]->Fms_Elev[j]->Fms_Slope[k]->LoVal +
						m * GetMoistCalcInterval(FuelSize,
								FM_INTERVAL_ASP);
					for (n = 0;
						n <
						Stations[sn].FMS[FuelSize][i]->Fms_Elev[j]->Fms_Slope[k]->Fms_Aspect[m]->NumFms;
						n++)
					{
						Cover = Stations[sn].FMS[FuelSize][i]->Fms_Elev[j]->Fms_Slope[k]->Fms_Aspect[m]->LoVal +
							n * GetMoistCalcInterval(FuelSize,
									FM_INTERVAL_COV);
						...
						if (cov->fms[n] == NULL)
						{
							...
							{
							case SIZECLASS_10HR:
								cov->fms[n] = Fms_Create10Hour("10hr"); break;
							case SIZECLASS_100HR:
								cov->fms[n] = Fms_Create100Hour("100hr"); break;
							case SIZECLASS_1000HR:
								cov->fms[n] = Fms_Create1000Hour("1000hr"); break;
							}
						}

						Radiate = SimpleRadiation(Date, Hour, Cloud, ElevInc,
									Slope, Aspect, Cover);
						...
						Stations[sn].CuumRain += (double)
							GetMoistCalcInterval(FuelSize,
															FM_INTERVAL_TIME) /
							24.0 * Rain;
						Fms_Update(cov->fms[n],
							GetMoistCalcInterval(FuelSize, FM_INTERVAL_TIME) /
							60.0,
						...
						i * MxDesc->NumElevs[sn][FuelSize] * MxDesc->NumSlopes[sn][FuelSize] * MxDesc->NumAspects[sn][FuelSize] * MxDesc->NumCovers[sn][FuelSize] +
							...
							m * MxDesc->NumCovers[sn][FuelSize] +
							n;
...
double FmsThread::SimpleRadiation(long date, double hour, long cloud,
	long elev, long slope, long aspect, long cover)
{
