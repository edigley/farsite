...
static const double small = 1.e-06, big = 1.e+06;
...
long BurnUp::GetSamplePoints(long Flaming, long NumberOfPts)//, double **xs, double **ys, long *numcalcs)
{
	...
	do
	{
		...
		{
			memmove(&x[NewPointPos + 1], &x[NewPointPos],
				(NumInSample - NewPointPos) * sizeof(double));
			memmove(&y[NewPointPos + 1], &y[NewPointPos],
				(NumInSample - NewPointPos) * sizeof(double));
			memmove(&pts[NewPointPos + 1], &pts[NewPointPos],
