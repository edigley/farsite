...
void FECalls::GetFireEnvironment(double SIMTIME, int type, long ReadNewData,
	long PointNum)
{
...
long FECalls::AllocFireMW(long NumPoints)
{
	if (!FireMW)
	{
		if ((FireMW = (EnvironmentData *)
			malloc(NumPoints * sizeof(EnvironmentData))) !=
			NULL)
			return 1;
	}
...
{
	double mf = ohmc / 100, Qig, ProbIgnite,
		FT = (5.0 / 9.0) * (FuelTemp - 32.0);  // convert to deg C

	Qig = 144.51 -
		.266 * FT -
		.00058 * pow2(FT) +
		(mf / 100) * (100 - FT) +
		18.54 * (1.0 - exp(-15.1 * mf)) +
		640 * mf;
...
void FireEnvironment::envt(double SIMTIME)
{
	...
	double emcmax, emcmin, humidmx, humidmn, tempmx, tempmn, dayhours,
		nitehours;
	...
	for (curdate = GetStartDate(); curdate <= lastdate; curdate++)
	{
		...
		if (AtmGridExists == 2)
		{
			AtmHumTemp(thisdate, curhour, &temp, &humid, &tempmn, &humidmn,
				&rain, &cloud);
			humidmx = humid;
			tempmx = temp;
		}
		...
		{
			humtemp(thisdate, curhour, &tempref, &humref, &elevref, &rain,
				&humidmx, &humidmn, &tempmx, &tempmn);
			//			rain/=100.0;
			sitespec(elevref,(long) elevation, tempref, &temp, humref, &humid);	
			// adjusts to site specific temp and relhum at a given pixel
		}
		...
		if (AtmGridExists < 2)
		{
			sitespec(elevref, (long)elevation, tempmn, &tempmn, humidmx, &humidmx);
			sitespec(elevref, (long)elevation, tempmx, &tempmx, humidmn, &humidmn);
		}
		if (shade < 70)
	...
	if (nearesthour != 1400)
	{
		while (curhour != nearesthour)	// calcs thmc for all hours since last day
		{
			...
			if (AtmGridExists == 2)
				AtmHumTemp(thisdate, curhour, &temp, &humid, &tempmn,
					&humidmn, &rain, &cloud);
			else
			{
				humtemp(thisdate, curhour, &tempref, &humref, &elevref, &rain,
					&humidmx, &humidmn, &tempmx, &tempmn);
				sitespec(elevref, (long)elevation, tempref, &temp, humref, &humid);	// adjusts to site specific temp and relhum at a given pixel
			}
			AirTemp = temp; AirHumid = humid;
			if (AtmGridExists == 0)
				windadj(thisdate, curhour, &cloud);			// retrieves wind information and adjusts to fuel-level conditions
	...
	if (SIMTIME < 241.0)
	{
		...
		ohmc = GetInitialFuelMoisture(ld.fuel, 1) -
			LagRate * (GetInitialFuelMoisture(ld.fuel, 1) - ohmc);
	}
	FuelTemp = temp;
	//	PtTemperature=temp;
	//	PtHumidity=humid;
...
void FireEnvironment::AtmHumTemp(int date, int hour, double* temp,
	double* humid, double* tempmin, double* humidmin, double* rain,
	long* cloud)
{
	long count = -1;
	...
	if (month != -1)						// if hit end of ATMDATA data
	{
		...
		do
		{
			...
			{
				GetAtmosphereGrid()->GetAtmosphereValue(ATMTEMP, XLocation,
										YLocation, count, &tempmin2);
				GetAtmosphereGrid()->GetAtmosphereValue(ATMHUMID, XLocation,
										YLocation, count, &humidmin2);
				if (tempmin1 == -1)
				{
					tempmin1 = tempmin2;
					humidmin1 = humidmin2;
		...
		while (hour >= hhour);
		--count;
		GetAtmosphereGrid()->GetAtmosphereValue(ATMTEMP, XLocation, YLocation,
								count, &tempx);
		GetAtmosphereGrid()->GetAtmosphereValue(ATMHUMID, XLocation,
								YLocation, count, &humidx);
		GetAtmosphereGrid()->GetAtmosphereValue(ATMRAIN, XLocation, YLocation,
								count, &ppt);
		*temp = (double) tempx;
		*humid = (double) humidx;
...
void FireEnvironment::humtemp(int date, int hour, double* tempref,
...
{
	long count = -1, ddate = -1, month, day, hmorn, haft, Tmin, Tmax, Hmax,
		Hmin;
	...
	ppt =(long) GetWeatherRain(StationNumber, count);
	...
	elref = (long)GetWeatherElev(StationNumber, count);
	if (hour > haft)
	{
		count++;
	...
	*tempref = double (Tmax + Tmin) /
		2.0 +
		(Thalf * sin(PI * (dtprime / dtmxmn - .5)));
	*humref = double (Hmax + Hmin) /
		2.0 +
		(Hhalf * sin(PI * (dtprime / dtmxmn - .5)));
	*elevref = elref;
	*tempmx = (double) Tmax;
	*tempmn = (double) Tmin;
	*humidmx = (double) Hmax;
	...
	// Determines actual date and time from SIMTIME variable
{
	...
	*hour = (int) hours;					// truncates time to nearest hour
	...
	hours = ((double) * hour) * 100.0 +
		(double) (GetStartHour() + GetStartMin());			 // elapsed time in hours
	days = hours / 2400.0;
	day = (int) days;
...
/* CALCULATES STANDARD DAILY FINE FUEL MOISTURE CONTENT */
{
	...
	else
	{
		if (mr > Ed)
		{
			X = .424 * (1 -
				pow((humid / 100.0), 1.7) +
				.088 * sqrt(fabs(twindspd)) * (1.0 - pow(humid / 100.0, 8)));
...
/* CALCULATES HOURLY FINE FUEL MOISTURE INDEX FROM CANADIAN FBPS, ROTHERMEL ET AL. 1986 */
{
	...
	mo = m = thmc;
	Ed = .942 * pow(fabs(humid), 0.679) +
		11 * exp((humid - 100.0) / 10.0) +
		.18 * (21.1 - Tc) * (1.0 - exp(-0.15 * humid));
	if (mo > Ed)
	{
		ka = .424 * (1 - pow(fabs(humid / 100), 0.17)) +
			.0694 * sqrt(fabs(twspd)) * (1 - pow((humid / 100), 8));
	...
	else
	{
		if (mo < Ed)
		{
			Ew = .618 * pow(fabs(humid), 0.753) +
				10 * exp((humid - 100.0) /
														10.0) +
				.18 * (21.1 - Tc) * (1 - exp(-0.15 * humid));
			if (mo < Ew)
			{
				kb = .424 * (pow(1.0 - ((100.0 - humid) / 100.0), 1.7)) +
					.0694 * sqrt(fabs(twspd)) * (1 -
					pow(((100.0 - humid) / 100.0),
														8));
...
void FireEnvironment::sitespec(long elevref, long elevx, double tempref,
	double* temp, double humref, double* humid)
...
/* FUNCTION TO REDUCE WINDSPEED (MI/HR) TO MIDFLAME OR VEGETATION HEIGHT */
{
	...
	if (ld.cover == 0 || canopyht == 0) // cover==99
	{
		if (ld.fuel > 0)
		{
			...
			case 13:
				htfuel = 3.0; break;
			default:
				htfuel = GetFuelDepth(ld.fuel - 14);  // retrieve from cust models
				if (htfuel == 0.0)
					htfuel = 0.01;
				break;
			}
		...
		m1 = (1 + .36 * (htfuel / htflame)) /
			(log((20 + .36 * htfuel) / (0.13 * htfuel)));
	...
	else
	{
		...
		m1 = .555 /
			(sqrt(canopyht * ffactor) * log((20 + .36 * canopyht) /
											(.13 * canopyht)));
...
double FireEnvironment::irradiance(long cloud, int date, int hour,
	double* shade)
