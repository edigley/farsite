    ...
    do
    {
        ...
        for (k = Begin; k < End; k++)
        {
            ...
            burnup->SetFireDat(5000,
                               ...
                               depth, 27.0, 1.8, 0.4, (double) BURNUP_TIMESTEP,
                                 ...
                                 double* ipoints, long NumIsects, long CurNumPts)
{
    ...
    for (i = 0; i < 2; i++)
    {
        ...
        {
            if (GetNumPoints(ring[i]->OriginalFireNumber) !=
    ...
    *Weight = *Flame = 0.0;
    ...
    if (ft_temp1 > 0.0)	// flaming still going on
    {
        NumFlameSpatCalcs = (long)
                            (ring->ElapsedTime /
                             (ring->perimpoints[j].hist.FlameTime / (double) PRECISION)) +
                            1;
...
double PFrontData::GetMax(long Species, long Phase)
{
    ...
    {
    case PF_FLAMING:
        MaxVal = MaxFlaming * GetFlameMult(Species);
        break;
    case PF_SMOLDERING:
        MaxVal = MaxSmolder * GetSmolderMult(Species);
        break;
    case PF_TOTAL:
