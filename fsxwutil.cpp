...
{
    if (NumCrossThreads == omp_get_max_threads())
        return true;

    CloseCrossThreads();
    crossthread = new CrossThread[omp_get_max_threads()];
    ...
    NumCrossThreads=omp_get_max_threads();
...
bool Intersections::CrossCompare(long* CurrentFire, long NextFire)
{
    ...
    do
    {
        ...
        //WaitForFarsiteEvents(EVENT_CROSS, threadct, true, 9999);
                ...
                count3++)	// FOR ALL FIRES CHECK FOR ALL CROSSES FROM THE FIRST ELEMENT
        {
            ...
            if (crossyn == 1)
            {
                ...
                if (numcross < CrossMemAlloc)
                {
                    ...
                    rcx3 = GetPerimeter1Value(NextFire, count5, RCXVAL);
            ...
            if (numcross > 0 && numcross < CrossMemAlloc)
            {
                if (NumTimes == 0)  // do this always for inward and outward fires
                {
                    ...
                    numcross = 0;
                         ...
                         inside * CheckReverse == 1)// && GetInout(*CurrentFire)==2)// reverse decision if
                {
                    ReorderPerimeter(*CurrentFire, begincross);  // reorder without origin inside loop
                    count1 = SpanAStart - 1;		   			// start again
                    numcross = 0;
    ...
    if (CrossFound)
    {
        ...
        for (StartCheck = 0; StartCheck < numcross; StartCheck++)
        {
                    ...
                    NextPoint++)
            {
                ...
                SetPerimeter2(NextPoint + NumNewPoints, xpt1, ypt1, ros1,
                              fli1, rcx1);
            }
            ...
            if (Isect1a == Isect2a)
            {
                ...
                if (dist1 > 1e-8)
                {
                    ...
                    if (fli1 < 0.0 || fli2 < 0.0)
                        fli3 *= -1.0;
                    SetPerimeter2(NumPointsWritten + NumNewPoints, xpt3, ypt3,
                                  ros3, fli3, rcx3);
                    NumNewPoints++;
                }
                else
        ...
        for (NextPoint = NumPointsWritten;
                NextPoint <= GetNumPoints(CurrentFire);
                NextPoint++)
        {
            xpt1 = GetPerimeter1Value(CurrentFire, NextPoint, XCOORD);
            ypt1 = GetPerimeter1Value(CurrentFire, NextPoint, YCOORD);
...
bool Intersections::MergeFire(long* CurrentFire, long NextFire)
{
    ...
    else   // merging 2 fires with internal loops formed
    {
        ...
        if (!inside)			  // if first point on fire1 is within 2nd fire
        {
            ...
            freadend = SpanEnd;
            fxend = numchek;
            ctfire1 = *CurrentFire;			// fxend=numchek;
            readnum = *CurrentFire;
            writenum = ctfire2;
            MergeWrite(fxend, freadstart, freadend, &xwrite);
