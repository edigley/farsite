...
void Embers::fuelcoefs(int fuel, double* coef, double* expon)
{
	...
	{
	case 1:
		*coef = 545; *expon = -1.21; break;
	case 2:
		*coef = 709; *expon = -1.32; break;
	case 3:
		*coef = 429; *expon = -1.19; break;
	case 4:
		*coef = 301; *expon = -1.05; break;
	case 5:
		*coef = 235; *expon = -0.92; break;
	case 6:
		*coef = 242; *expon = -0.94; break;
	case 7:
		*coef = 199; *expon = -0.89; break;
	case 8:
		*coef = 262; *expon = -0.97; break;
	case 9:
		*coef = 1121; *expon = -1.51; break;
	case 10:
		*coef = 224; *expon = -0.92; break;
	case 11:
		*coef = 179; *expon = -0.81; break;
	case 12:
		*coef = 163; *expon = -0.78; break;
	case 13:
		*coef = 170; *expon = -0.79; break;
	};
}
...
void Embers::Loft(double CFlameLength, double CFBurned, double CrownHeight,
	...
	double curtime)
{
	...
	for (long count = 1; count < 17; count++)
	{
		Diameter = .005 * count;
		switch (SpotSource)
		{
		...
		if (ZF > 0) 								// if ember is lofted by flame
		{
			...
			{
				//NextEmber = (emberdata *)
				//	CurEmber->next = (emberdata *) new emberdata;// GlobalAlloc(GMEM_FIXED |  GMEM_ZEROINIT, 1*sizeof(emberdata));
				emberdata* t_next;
...
void Embers::Plume(double CFlameLength, double CFBurned)
{
	...
	if (SpotSource <= 1)			// active crown fire or torching trees
	{
		...
		FrontDist = sqrt(pow2(xdiffl) + pow2(ydiffl)) + sqrt(pow2(xdiffn) +
															pow2(ydiffn));	// linear frontal fire distance surrounding x,y coord
		if (SpotSource == 0)			  		// if active crown fire,
		{
			...
			if (tnum < 1)
				tnum = 1;   			  // minimum number of trees is 1
...
void Embers::Flight(double CurTime, double EndCurTimeStep)
{
	...
	long IgProb = (long)PercentIgnition(GETVAL) * (long)100;		// in FSXW.CPP
	long ProbSpot;
	...
	for (count = 0; count < NumEmbers; count++)		  // for all particles
	{
		...
		//	continue;
		//--------------------------------------------------------
		ProbSpot = (long)rand2(&idum) * (long)10000;//rand()%1000;
		if (ProbSpot > IgProb)
			continue;
		//--------------------------------------------------------
		...
		do
		{
			ztest1 = voo * tao * ((elapsedtime * 60.0) /
				tao -
				.5 * pow2((elapsedtime * 60.0) /
																		tao));// compute test vertical drop of ember over sstep (0.25 min)
			ztest2 = voo * tao * (((sstep + elapsedtime) * 60.0) /
				tao -
				.5 * pow2(((sstep + elapsedtime) * 60.0) /
						tao));// compute test vertical drop of ember over sstep (0.25 min)
			...
			for (PassA = 0; PassA < 3; PassA++)
			{
				...
				for (PassB = 0; PassB < 2; PassB++)
				{
					...
					if (mZt > 1.0)								// 1.0 meter tolerance
					{
						...
						if (Z1AboveGround > 0)
						{
							...
							{
								dxdt = UH / 2.03 * log(ZAvgAboveGround /
													(.1313 * eH));		// dxdt is ft/s
				...
				if (PassA > 7)
					break;
				tseast = seast -
					mXt * MetricResolutionConvert() * sin(rwinddir);		// temporary position change, will be permanent if Z iterations ok
				tsnorth = snorth -
					mXt * MetricResolutionConvert() * cos(rwinddir);
				if (tseast >= GetHiEast() ||
					tseast <= GetLoEast() ||
					tsnorth >= GetHiNorth() ||
					tsnorth <= GetLoNorth())
		...
		if (mZt >= -1.0 && mZt <= 1.0 && curtime < EndCurTimeStep)// && NumSpots<19000)  // NumSpot restriction is TEMPORARY
		{
			if (mech.ld.fuel > 0)						// if not a rock or lake etc.
			{
				...
				{
					//NextSpot = (spotdata *)
					//	CurSpot->next = (spotdata *) new spotdata;//GlobalAlloc(GMEM_FIXED |  GMEM_ZEROINIT, 1*sizeof(spotdata));
					spotdata* t_next;
					t_next = new spotdata;
					CurSpot->next = t_next;
					NextSpot = t_next;
	...
	// for torching trees only, function used by torcheight
{
	...
	tp = a /
		(pow2(pt8vowf) * pt8vowf) * (log(Temp) -
		pt8vowf * (r - 1.0) -
		.5 * pow2(pt8vowf) * (r2 - 1.0));
	tT = tp - tT;
	...
	// for piles only (ALBINI 1981)
{
	...
	double MaxRhs = (.0078 * SteadyHeight) * (8.0 / 3.0) * (1.0 -
		0.625 * pow(0.625,
					5.0 /
					3.0)) * pow(0.625,
																						2.0 /
	...
	if (lhs <= MaxRhs) 	// if particle can be lifted by max buoyant
	{
		...
		OldRhs = rhs = (.0078 * SteadyHeight) * (8.0 / 3.0) * (1.0 -
			0.625 * pow(1.0 /
						ZoZf,
						5.0 /
						3.0)) * pow(1.0 /
		...
		while (fabs(rhs - lhs) > .01)		//TOLERANCE SET FOR 1/100 FT
		{
			...
			else
			{
				...
				OldRhs = rhs;
				rhs = (.0078 * SteadyHeight) * (8.0 / 3.0) * (1.0 -
					0.625 * pow(1.0 /
								ZoZf,
								5.0 /
								3.0)) * pow(1.0 /
...
double Embers::VertWindSpeed(double HeightAboveGround, double TreeHt)
{
	...
	{
		UH = (mech.lmw.tws * 1.4667) /
			(log((20.0 + .36 * TreeHt) / (.1313 * TreeHt)));   // twindspd converted to feet/s from mph
